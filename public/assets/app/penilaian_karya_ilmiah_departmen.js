...
function showData() {
    console.log(idDepartmen);
    table = $("#datatable").DataTable({
        processing: true,
        serverSide: true,
        ajax: `/api/karya-ilmiah/departmen/${idDepartmen}`,
        autoWidth: false,
        columnDefs: [
            {
                targets: -1,
        ...
        columns: [
            ...
            },
            {
                ...
                orderable: false,
                searchable: false,
                render: function (penilaianKaryaIlmiah) {
                    const isEvaluated = penilaianKaryaIlmiah.length > 0;
                    if (isEvaluated) {
                        const hasEvaluations = penilaianKaryaIlmiah.some(
                        ...
                        return "<span class='text-danger'>Belum Dinilai</span>";
                    }
                },
            },
            {
        ...
        success: function (response) {
            ...
            if (userPenilaian) {
                ...
                statusButton = `
                    ...
                    <input id="skor_departmen" type="number" step="0.01" value="${userPenilaian.skor_departmen}" class="form-control" />
                ...
                statusButton = `
                    <label for="skor">Skor <i class="text-danger">*</i></label>
                    <input id="skor_departmen" type="number" step="0.01" class="form-control" />
...
function reviewKaryaIlmiah(id) {
    const skor_departmen = $("#skor_departmen").val();
    if (!skor_departmen) {
        return showToastErrorAlert("Skor wajib diisi");
    }
    let data = {
        skor_departmen: skor_departmen,
    };
    $.ajax({
        type: "PATCH",
        url: `/api/karya-ilmiah/review-departmen/${id}`,
