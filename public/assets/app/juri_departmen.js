...
function showData() {
    table = $("#datatable").DataTable({
        processing: true,
        serverSide: true,
        ajax: "/api/user/juri-departmen/data",
        autoWidth: false,
        columnDefs: [
            {
                targets: -1,
...
function addModal() {
    const modalHeader = "Tambah Juri Departmen";
    const modalBody = `
        <form class="form form-horizontal">
            <div class="form-body">
                <div class="row">
                   <input type="hidden" id="id_departmen" value="${idDepartmen}">
                   <input type="hidden" id="id_role" value="7">
...
function editModal(id) {
    $.ajax({
        ...
        url: `/api/user/${id}`,
        success: function (response) {
            let { id, id_role, id_departmen, name, username, status } =
                response.data;
            const modalHeader = "Ubah Data Juri Departmen";
            const modalBody = `
                <form class="form form-horizontal">
                    <div class="form-body">
                        <div class="row">
                            <input type="hidden" id="id_departmen" value="${id_departmen}">
...
function deleteModal(id, name) {
    const modalHeader = "Hapus Juri departmen";
    const modalBody = `Apakah anda yakin ingin menghapus juri departmen ${name}?`;
...
function update(id) {
    ...
    if (!id_departmen) {
        return showToastErrorAlert("departmen wajib diisi");
    }
    let data = {
        id_departmen: id_departmen,
        username: username,
